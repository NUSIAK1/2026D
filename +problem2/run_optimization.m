...
defaults = struct('TimeLimit_s',3600,'NumRuns',10,'MaxIterations',100000, ...
    ...
    'MaxTimeLimit_s',0,'AdaptiveExtend',false,'AdaptiveWindow_s',1200);
...
assert(config.MaxTimeLimit_s >= config.TimeLimit_s && config.MaxTimeLimit_s <= 7200, ...
    '自适应预算上限必须不小于基准预算且不超过 7200 秒。');
...
summary = struct('Passed',true,'SeedCount',size(oldObjectives,1), ...
    ...
    'BaseLimit_s',config.TimeLimit_s, ...
    'MaxLimit_s',config.MaxTimeLimit_s, ...
    'AdaptiveExtend',config.AdaptiveExtend, ...
    'LastExtremeImprove_s',result.AdaptiveBudget.LastExtremeImprove_s);
